...
export const saveSentEmails = (request, response) => {
    ...
    try {
        ...
        response.status(200).json("Mail is sent succcesfully");
...
export const moveEmailsToBin = async (request, response) => {
    try {
        ...
        return response.status(200).json(' moved to bin successfullly');
...
export const starredMails = async (request, response) => {
    try {
        await Email.updateOne({ _id: request.body.id }, { $set: { starred: request.body.value } });
        return response.status(200).json(' Updated as starred successfullly');
...
export const deleteMails = async (request, response) => {
    try {
        ...
        return response.status(200).json('Delete Successfully');
